...
pub async fn http_request_with_headers(
    ...
    method: &str,
) -> Result<(Bytes, u16, HashMap<String, String>), InternalError> {
...
pub async fn http_get_request(
    url: &str,
    headers: &HashMap<String, String>,
) -> Result<(Bytes, u16), InternalError> {
    let method = "GET";
...
pub async fn http_request(
    ...
    let window = web_sys::window()
        .ok_or(InternalError::String("No window available".to_string()))?;
    ...
    let headers_map =
        Headers::new().map_err(|e| InternalError::Js(e.into()))?;
    for (key, value) in headers {
        headers_map
            .set(key, value)
            .map_err(|e| InternalError::Js(e.into()))?;
    }
    ...
    if status >= 200 && status < 300 {
        ...
        Err(InternalError::String(error_message))
    }
}
