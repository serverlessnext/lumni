...
use ::xlatti::{ListObjectsResult, ObjectStoreHandler, EnvironmentConfig, AWS_DEFAULT_REGION};
...
impl _Client {
    ...
    fn cli(&self, args: &PyList) -> PyResult<()> {
        let mut args: Vec<String> = args
            ...
            .collect();
        args.insert(0, "xlatti".to_string());
        run_cli(args);
        Ok(())
    }
    ...
    fn list_objects(
        ...
        match result {
            Ok(Some(list_objects_result)) => match list_objects_result {
                ...
                _ => {
                    let xlatti_error = LakestreamError::new_err(format!("Error listing objects: {}", "Unknown error"));
                    Err(xlatti_error)
                },
            },
            Ok(None) => Ok(PyList::empty(py).to_object(py)),
            Err(err) => {
                let xlatti_error = LakestreamError::new_err(format!("Error listing objects: {}", err));
                Err(xlatti_error)
            },
        }
    }
    ...
    fn list_buckets(
        ...
        match result {
            Ok(Some(list_objects_result)) => match list_objects_result {
                ...
                _ => {
                    let xlatti_error = LakestreamError::new_err(format!("Error listing buckets"));
                    Err(xlatti_error)
                },
            },
            Ok(None) => Ok(PyList::empty(py).to_object(py)),
            Err(err) => {
                let xlatti_error = LakestreamError::new_err(format!("Error listing buckets: {}", err));
                Err(xlatti_error)
            },
        }
    }
    ...
    fn get_object(&self, py: Python, uri: String) -> PyResult<PyObject> {
        ...
        match result {
            ...
            Err(err) => {
                let xlatti_error = LakestreamError::new_err(format!("Error getting object: {}", err));
                Err(xlatti_error)
            },
        }
    }
